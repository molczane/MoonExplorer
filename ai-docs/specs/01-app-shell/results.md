...
| **SC-004** Settings opens placeholder | ✓ code-complete | About → Settings → SettingsSheet sequential flow wired in `MoonExplorerScreen`. Visual UX pending hardware. **Update 2026-04-30:** the placeholder body got its first real toggles in `07-celestial-background` (T730) — `Show stars` + `Show sun` switches bound to `MoonRenderState`. The "Coming soon" copy is gone; the entry-affordance + sequential-stack contract (this SC's actual subject) still holds. See [`../07-celestial-background/results.md`](../07-celestial-background/results.md). |
